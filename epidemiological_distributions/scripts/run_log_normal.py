...
model_district = pystan.StanModel(file = SCRIPTS_PATH + 'model_log_normal_district.stan')
...
model = pystan.StanModel(file = SCRIPTS_PATH + 'model_log_normal_pool.stan')
# 5. Run model
ut.get_posteriors_pooling(all_dfs, columns, model, 'log_normal', params, district_posteriors, n_strats, strat_name)

print('Log normal model fits done')
