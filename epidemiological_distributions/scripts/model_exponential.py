...
model_district = pystan.StanModel(model_code=code_exponential_district)
...
def get_posteriors_exponential(param_list):
    district_posteriors = {}
...
district_posteriors_exponential = get_posteriors_exponential(['beta'])
...
model {
    // likelihood
    for (i in 1:N){
            X[i] ~ exponential(beta[INSERT_STRAT[i]]);
    }
    // priors
...
def fit_partial_pooling(stan_code, df, col, beta, n_strats, strat_name):
    ...
    params_columns = posterior_df.columns.str.startswith('beta')\
                   + posterior_df.columns.str.startswith('sigma_')
    posterior_df = posterior_df.loc[:,params_columns]
    return posterior_df

strat_posteriors_exponential = {}
for i in range(len(columns)):
    df = all_dfs[i]
    col = columns[i]
    print(col)
    stan_code = code_pp_exponential
    beta = district_posteriors_exponential[col]['beta'].values.mean()
    posterior = fit_partial_pooling(stan_code, df, col, beta, len(strat_), strat)
    # add national estimates
    posterior = pd.concat([posterior, district_posteriors_exponential[col]], axis=1, sort=False)
    strat_posteriors_exponential.update({col: posterior})
    # save the output
    posterior.to_csv(OUT_PATH + col +f'-samples-exponential_{strat}.csv', index=False)
    strat_posteriors_exponential.update({col: posterior})
    del posterior, df
